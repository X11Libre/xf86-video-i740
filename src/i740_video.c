...
i740fb_overlay_off(ScrnInfoPtr pScrn)
{
...
__inline__ static void
i740fb_overlay_set(ScrnInfoPtr pScrn, I740PortPrivPtr pPriv,
                   unsigned long mem1, unsigned long mem2,
                   /* source      image size */
                   unsigned long isrc_w, unsigned long isrc_h,
                   /* destination image size */
                   unsigned long idst_w, unsigned long idst_h,
                   /* destination image pos to display */
                   unsigned long ddst_x, unsigned long ddst_y,
                   /* destination image size to display allows trunc... */
                   unsigned long ddst_w, unsigned long ddst_h,
                   unsigned long pitch, int flip, unsigned char vd_mod)
{
    I740Ptr pI740 = I740PTR(pScrn);
    ...
    {
        unsigned short v = ddst_x;

        i740_wc(fb_p, MRX, 0x2B, v >> 8);
        i740_wc(fb_p, MRX, 0x2A, v >> 0);
    }
    {
        unsigned short v = ddst_x + ddst_w - 1;

        i740_wc(fb_p, MRX, 0x2D, v >> 8);
        i740_wc(fb_p, MRX, 0x2C, v >> 0);
    }
    {
        unsigned short v = ddst_y;

        i740_wc(fb_p, MRX, 0x2F, v >> 8);
        i740_wc(fb_p, MRX, 0x2E, v >> 0);
    }
    {
        unsigned short v = ddst_y + ddst_h - 1;

        i740_wc(fb_p, MRX, 0x31, v >> 8);
        i740_wc(fb_p, MRX, 0x30, v >> 0);
    }
    ...
    i740_wc(fb_p, MRX, 0x1E,
            (idst_w > isrc_w ? 0x04 : 0x00) |
            (idst_h > isrc_h ? 0x08 : 0x00));
    i740_wc(fb_p, MRX, 0x1F,
            (idst_w > isrc_w ? 0x20 : 0x00) |
            ((idst_h > isrc_h) && (pitch <= 720 * 2) ? 0xC0 : 0x00) |
            (vd_mod & 0x1F));
...
__inline__ static void
i740fb_colorkey(ScrnInfoPtr pScrn, unsigned long key)
{
    I740Ptr pI740 = I740PTR(pScrn);
...
static void
I740ResetVideo(ScrnInfoPtr pScrn)
{
    I740Ptr pI740 = I740PTR(pScrn);
...
static void
I740ClipVideo(BoxPtr dst, INT32 *x1, INT32 *x2, INT32 *y1, INT32 *y2,
                     ...
                     INT32 value, pointer data)
{
    I740PortPrivPtr pPriv = (I740PortPrivPtr) data;
    I740Ptr pI740 = I740PTR(pScrn);
    I740OverlayRegPtr overlay =
        (I740OverlayRegPtr) (pI740->FbBase + pI740->OverlayStart);
...
static void
I740QueryBestSize(ScrnInfoPtr pScrn,
                  Bool motion,
                  ...
                  pointer data)
{
    ...
    xf86DrvMsg(pScrn->scrnIndex, X_CONFIG,
               "I740QueryBestSize entered %d %d %d %d\n",
...
static void
I740CopyMungedData(ScrnInfoPtr pScrn,
                   unsigned char *src1, unsigned char *src2,
                   unsigned char *src3, int srcPitch, int srcPitch2,
                   int dstPitch, int h, int w, long scalex, long scaley)
{
    I740Ptr pI740 = I740PTR(pScrn);
    I740PortPrivPtr pPriv = pI740->adaptor->pPortPrivates[0].ptr;
    CARD32 *dst;
            ...
            for (int i = 0; i < dstw; i++) {
                ...
                dst[i] =
                    (((src1_[srci_rnd << 1] | (src1_[(srci_rnd << 1) + 1] << 16)
                       | (src3_[srci_rnd] << 8) | (src2_[srci_rnd] << 24))) >> 1
                     & 0x7F7F7F7FL)
                    +
                    (((src1_[srci2_rnd << 1] |
                       (src1_[(srci2_rnd << 1) + 1] << 16) |
                       (src3_[srci2_rnd] << 8) |
                       (src2_[srci2_rnd] << 24))) >> 1 & 0x7F7F7F7FL);
                srci += scalex;
            }
            dst += dstPitch;
            dstj += scaley;
...
__inline__ static void
I740CopyPackedData(ScrnInfoPtr pScrn,
                   unsigned char *buf,
...
__inline__ static void
I740DisplayVideo(ScrnInfoPtr pScrn, int id, short width, short height,
                 ...
                 unsigned char vd_mod)
{
    ...
    i740fb_overlay_set(pScrn, pPriv, pPriv->YBuf0offset, pPriv->YBuf1offset,
                       ...
                       dstPitch, (pPriv->currentBuf != 0), vd_mod);

/* needed to reset properly the i740 board after switching from framebuffer */
    i740fb_colorkey(pScrn, pPriv->colorKey);
}

static FBLinearPtr
I740AllocateMemory(ScrnInfoPtr pScrn, FBLinearPtr linear, int size)
{
    ScreenPtr pScreen;
    ...
    if (linear) {
        ...
        if (xf86ResizeOffscreenLinear(linear, size)) {
            xf86DrvMsg(pScrn->scrnIndex, X_CONFIG,
                       "I740AllocateMemory resized to %d - %p\n",
                       (int) size, (void *) linear);     /* ### */
            return linear;
        }

        xf86DrvMsg(pScrn->scrnIndex, X_CONFIG,
                   "I740AllocateMemory free %p - %d < %d\n",
    ...
    new_linear =
        xf86AllocateOffscreenLinear(pScreen, size, 4, NULL, NULL, NULL);
    ...
    if (!new_linear) {
        ...
        if (max_size < size) {
            xf86DrvMsg(pScrn->scrnIndex, X_CONFIG,
                       "I740AllocateMemory can't purge %d < %d\n",
                       (int) max_size, (int) size);     /* ### */
            return NULL;
        }

        xf86DrvMsg(pScrn->scrnIndex, X_CONFIG,
                   "I740AllocateMemory purged %d\n", (int) max_size); /* ### */
        xf86PurgeUnlockedOffscreenAreas(pScreen);
        new_linear = xf86AllocateOffscreenLinear(pScreen, size, 4,
                                                 NULL, NULL, NULL);
    }
    xf86DrvMsg(pScrn->scrnIndex, X_CONFIG,
               "I740AllocateMemory allocated %d - %p\n",
...
static int
I740PutImage(ScrnInfoPtr pScrn,
...
static int
I740QueryImageAttributes(ScrnInfoPtr pScrn, int id, unsigned short *w,
                         unsigned short *h, int *pitches, int *offsets)
{
    int size, tmp;

    if (*w > IMAGE_MAX_PHY_WIDTH)
        *w = IMAGE_MAX_PHY_WIDTH;
    if (*h > IMAGE_MAX_PHY_HEIGHT)
        *h = IMAGE_MAX_PHY_HEIGHT;
    ...
    if (pPriv->videoStatus & TIMER_MASK) {
        ...
        if (pPriv->videoStatus & OFF_TIMER) {
            if (pPriv->offTime < currentTime.milliseconds) {
                /*xf86DrvMsg(pScrn->scrnIndex, X_CONFIG,
                  "I740BlockHandler: OFF_TIMER expired\n"); *//* ### */
            ...
            if (pPriv->freeTime < currentTime.milliseconds) {
                if (pPriv->linear) {
                    /*xf86DrvMsg(pScrn->scrnIndex, X_CONFIG,
                      "I740BlockHandler: FREE_TIMER expired\n"); *//* ### */
                    xf86FreeOffscreenLinear(pPriv->linear);
                    pPriv->linear = NULL;
                }
                pPriv->videoStatus = 0;
...
static int
I740AllocateSurface(ScrnInfoPtr pScrn, int id,
                    ...
                    XF86SurfacePtr surface)
{
    ...
    xf86DrvMsg(pScrn->scrnIndex, X_CONFIG,
               "I740AllocateSurface entered %d %d\n",
               (int) w, (int) h);    /* ### */
...
static int
I740StopSurface(XF86SurfacePtr surface)
{
...
static int
I740FreeSurface(XF86SurfacePtr surface)
{
    OffscreenPrivPtr pPriv = (OffscreenPrivPtr) surface->devPrivate.ptr;
...
static int
I740GetSurfaceAttribute(ScrnInfoPtr pScrn, Atom attribute, INT32 *value)
{
    return I740GetPortAttribute(pScrn, attribute, value, 0);
}

static int
I740SetSurfaceAttribute(ScrnInfoPtr pScrn, Atom attribute, INT32 value)
{
    return I740SetPortAttribute(pScrn, attribute, value, 0);
}

static int
I740DisplaySurface(XF86SurfacePtr surface, short src_x, short src_y,
...
static void
I740InitOffscreenImages(ScreenPtr pScreen)
{
...

    {
        ...
        xf86DrvMsg(pScrn->scrnIndex, X_CONFIG,
                   "I740InitOffscreenImages entered\n");    /* ### */
    }
...
I740SetupImageVideo(ScreenPtr pScreen)
{
...
#define NUM_FORMATS (sizeof(i740vid_Formats) / sizeof(XF86VideoFormatRec))
    ...
    {
        const int n =
            sizeof(XF86VideoAdaptorRec) + sizeof(I740PortPrivRec) +
            sizeof(DevUnion);
...
void
I740InitVideo(ScreenPtr pScreen)
{
    ScrnInfoPtr pScrn = xf86ScreenToScrn(pScreen);
        ...
        if (newAdaptor) {
            if (!num_adaptors) {
                ...
                if ((adaptors_newptrs =
                    ...
                    memcpy(adaptors_newptrs, adaptors_oldptrs,
                    ...
                    xf86XVScreenInit(pScreen, adaptors_newptrs,
                                     num_adaptors + 1);
